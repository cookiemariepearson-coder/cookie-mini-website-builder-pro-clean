...
test('website and standalone video access remain server verified', async () => {
  ...
  assert.match(videoAccess, /purchaseEmailHash: videoEmailHash\(purchaseEmail\)/);
  assert.match(videoAccess, /claimStandalonePurchase/);
  ...
  assert.match(studio, /<h1>Create Your AI Video<\/h1>/);
  ...
  assert.match(studio, /openAccountModal\(\{ mode, destination: accountDestination\(\) \}\)/);
  assert.match(studio, /body: JSON\.stringify\(\{ mode: preferWebsite \? 'website' : 'account', slug \}\)/);
  assert.equal((studio.match(/'Create My Video'/g) || []).length, 1);
  assert.doesNotMatch(studio, />Verify Website Plan<|>Secure Website-Plan Sign-In<|>Verify License</);
  ...
  assert.match(studio, /if \(!storageReady\) return/);
  ...
  assert.match(videoCheckout, /window\.location\.assign\(checkoutUrl\)/);
  ...
  assert.match(videoCheckout, /href="\/video-studio\?claim=1">\s*Return to AI Video Studio &amp; Verify License/);
  ...
  assert.match(homepage, /href="\/video-studio\?intent=purchase">Start AI Video Studio/);
  assert.match(checkoutSuccess, /href="\/video-studio\?claim=1">Open AI Video Studio &amp; Verify License/);
});
