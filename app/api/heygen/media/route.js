...
export async function GET(request) {
  try {
    ...
    if (!access) return NextResponse.json({ ok: false, error: 'Verified video access is required.' }, { status: 401 });
    ...
    if (!authorized.ok) return NextResponse.json({ ok: false, error: 'This video does not belong to your verified access.' }, { status: authorized.status });

    const url = new URL(request.url);
    const jobId = String(url.searchParams.get('jobId') || '').trim();
    const kind = url.searchParams.get('kind') === 'thumbnail' ? 'thumbnail' : 'video';
    if (!/^[a-zA-Z0-9-]{8,100}$/.test(jobId)) return NextResponse.json({ ok: false, error: 'Saved video was not found.' }, { status: 404 });
    ...
    if (stored.missing || !stored.ok) {
      ...
      return NextResponse.json({ ok: false, error: 'Video media is temporarily unavailable.' }, { status: 503 });
    }
    if (!stored.job) return NextResponse.json({ ok: false, error: 'Saved video was not found.' }, { status: 404 });
    if (!videoJobBelongsToAccess(stored.job.website_slug, authorized.slug)) return NextResponse.json({ ok: false, error: 'This video does not belong to your verified access.' }, { status: 403 });
    ...
    if (!providerUrl || providerUrl.protocol !== 'https:') return NextResponse.json({ ok: false, error: 'This video is not ready yet.' }, { status: 404 });
    ...
    if (!upstream.ok || !upstream.body) {
      ...
      return NextResponse.json({ ok: false, error: 'Video media could not be loaded. Refresh the status and try again.' }, { status: 502 });
    }
    ...
    return NextResponse.json({ ok: false, error: 'Video media could not be loaded.' }, { status: 500 });
  }
}
