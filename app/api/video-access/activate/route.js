...
import { claimStandalonePurchase, latestStandalonePurchase } from '../../../../lib/videoPurchaseClaim';
...
async function verifiedStandaloneResponse(supabase, namespace, ownerId) {
  const { count, error } = await supabase
    .from('heygen_video_jobs')
    .select('id', { count: 'exact', head: true })
    .eq('website_slug', namespace);
  ...
  return privateResponse({
    ...
    verified: true,
    ...entitlement,
    token: createVideoAccessToken({ kind: 'standalone', namespace, ownerId }),
    access: 'Standalone AI Video Studio — 1 real video',
    message: entitlement.generationAllowed
      ? 'Your video credit is ready.'
      : 'Your included video credit has been used. Open Video Results or buy another video.'
  });
}
...
export async function POST(request) {
  try {
    ...
    if (licenseKey) {
      const limited = rateLimit(request, { name: 'video-access', limit: 10, windowMs: 15 * 60 * 1000, subject: `owner:${owner.user.id}:${privateLicenseSubject(licenseKey)}` });
      ...
      const claimed = await claimStandalonePurchase({
        ownerId: owner.user.id,
        namespace,
        purchaseEmailHash: videoEmailHash(purchaseEmail),
        client: owner.supabase
      });
      if (!claimed.ok) {
        return privateResponse({ ok: false, error: 'That purchase is already connected to another customer account.' }, 403);
      }
      return verifiedStandaloneResponse(owner.supabase, namespace, owner.user.id);
    }

    const limited = rateLimit(request, { name: 'video-access', limit: 10, windowMs: 15 * 60 * 1000, subject: `owner:${owner.user.id}` });
    if (!limited.ok) return rateLimitResponse(limited, 'Please wait before verifying access again.');

    if (body.mode === 'account' || body.mode === 'website') {
      if (body.mode === 'account') {
        const standaloneNamespace = await latestStandalonePurchase(owner.user.id, owner.supabase);
        if (standaloneNamespace) return verifiedStandaloneResponse(owner.supabase, standaloneNamespace, owner.user.id);
      }
      ...
      if (!eligible.length) {
        ...
        return privateResponse({
          ...
          message: 'Your account is ready. Buy one video to continue.'
        });
      }
