...
export default function AdminVideoCredits(){
 ...
 async function requestOwnerLink(){setLoading(true);const res=await fetch('/api/auth/admin/request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})});const data=await res.json();setMessage(data.message||data.error||'Could not send owner sign-in link.');setLoading(false);}
