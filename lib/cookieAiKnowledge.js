...
If the customer answers with short replies like "cooking" or "a cookbook", keep the plan-matching conversation going instead of changing topics.
...
function textFromMessages(history = [], message = '') {
  const pieces = [];
...
function userOnlyText(history = [], message = '') {
  ...
  for (const item of history) {
    if ((item.role || 'user') !== 'assistant') {
      pieces.push(item.content || item.text || '');
    }
  }
  pieces.push(message);
  return normalizeText(pieces.join(' '));
...
export function isActivePlanConsultation(message = '', history = []) {
  const combined = textFromMessages(history, message);
  const current = normalizeText(message);
  const shortReply = current.split(/\s+/).filter(Boolean).length <= 5;
  ...
  const looksLikePlanDetail =
    /(cooking|cookbook|recipe|recipes|food|hair|beauty|salon|cleaning|catering|restaurant|coach|course|digital product|book|ebook|photos|videos|book now|buy now|order|quote|ai video|small launch|fuller website|full website)/.test(current);
  ...
  return hasPlanStart && !unrelated && (shortReply || looksLikePlanDetail || classifyIntent(current) === 'plan_help');
}
...
export function getPlanProfile(message = '', history = []) {
  ...
  if (/(cookbook|recipe book|ebook|e-book|digital cookbook|recipes)/.test(text)) {
  ...
  const needsBuy = /(buy|sell|selling|gumroad|checkout|product|cookbook|ebook|e-book|digital product|payment)/.test(text);
  const needsOrder = /(order|food order|menu|plates|catering|restaurant|meal prep)/.test(text);
  const needsBook = /(book|booking|appointment|calendar|calendly|class|consultation|hair|salon|lashes|nails)/.test(text);
  const needsQuote = /(quote|estimate|request quote|service|cleaning|repair|lawn)/.test(text);
  const needsContact = /(call|text|email|contact)/.test(text);

  const mentionsAction = needsBuy || needsOrder || needsBook || needsQuote || needsContact;

  const mediaYes = /(photo|photos|image|images|gallery|video|videos|media|cover|pictures)/.test(text);
  const mediaNo = /(no photo|no photos|no video|no videos|no media|do not need photos|don't need photos)/.test(text);
  const mediaKnown = mediaYes || mediaNo;

  const aiYes = /(ai video|video studio|heygen|need ai|want ai|included ai)/.test(text);
  const aiNo = /(no ai|do not need ai|don't need ai|without ai|not ai)/.test(text);
  const aiKnown = aiYes || aiNo;

  const small = /(small|simple|launch page|basic|test)/.test(text);
  const full = /(full|fuller|complete|all sections|more sections|full website|website hub)/.test(text);
...
Home, About/Product, Contact or Order / Book / Buy.`;
  ...
  if (ai.aiYes) {
    if (size.full || actions.needsBuy || actions.needsOrder || actions.needsBook || actions.needsQuote) {
...
For your ${business}, I’d use sections like:
Home, About, Products/Menu, Gallery, FAQ, and Order / Book / Buy.
