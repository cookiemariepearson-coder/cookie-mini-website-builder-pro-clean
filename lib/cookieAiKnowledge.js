...
export function continuePlanFlow(message = '', previousState = {}) {
  ...
  if (!state.active) {
    return startPlanFlow();
  }

  // Apply the current answer to the current step only once.
  if (state.step === 'business') {
    ...
    if (includesAny(current, ['small', 'simple', 'launch', 'basic', 'test'])) state.size = 'small';
  ...
  // If an answer was unclear, ask the same question with a clearer nudge.
...
export function recommendPlan(state = {}) {
  ...
  if (!needsMedia && !wantsAi && s.size === 'small' && (action === 'none' || action === 'contact')) {
    plan = 'Free Launch Page';
    why = [
    ...
    plan = 'Business';
    why = [
      'You need a fuller business setup',
    ...
    plan = 'Starter Pro';
    why = [
      'You need a clean small business/product site',
      'You can upload cookbook images, recipe previews, covers, or promo graphics',
  ...
  if (plan === 'Free Launch Page') {
    return `Based on what you told me, I’d recommend the **Free Launch Page** for ${businessLabel}.

Why:
- ${why.join('\n- ')}

Good starter setup:
Home, About/Product, and Contact or Order / Book / Buy.`;
  }

  if (plan === 'Starter Pro') {
    return `Based on what you told me, I’d recommend **Starter Pro** for ${businessLabel}.

Why:
- ${why.join('\n- ')}

Best setup:
Home, About/Product, Gallery or Products, and Order / Book / Buy.

Choose **Business** instead if you want AI Video Studio included, more sections, or more customer action buttons.`;
  }

  return `Based on what you told me, I’d recommend the **Business plan** for ${businessLabel}.
...
Upgrade to **Premium** only if you want all built-in sections, up to 8 customer action buttons, and the most flexibility.`;
}
