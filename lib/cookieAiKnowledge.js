...
Cookie's AI Video Studio standalone: $5 one-time access for hooks, scripts, captions, scenes, prompts, content planning, and exactly 1 real AI-generated video after the purchase is verified.
`
  },
  {
...
If the user wants lower cost and only needs one page plus AI help and one real generated video, suggest Starter Pro + the $5 standalone AI Video Studio.
...
`
  },
  {
    ...
    content: `
Standalone AI Video Studio is $5 one-time and includes video planning tools plus exactly 1 real AI-generated video after the purchase is verified. A customer account is required before checkout; direct customers to /video-studio so they can create an account or sign in first.
...
Generation remains subject to provider processing, moderation, and technical availability, so a successful render or specific result is not guaranteed.
`
  },
  {
    title: 'Safety Rules',
...
export function classifyIntent(message = '', pagePath = '') {
  ...
  if (/(plan|price|pricing|cost|starter plan|business plan|premium plan|free plan|choose a plan|which plan|fits me|fit me|best plan|right plan|recommend a plan|pick a plan|one page plan|1 page plan)/.test(text)) return 'plan_help';
...
function planAnswerFromMessage(message = '') {
  const text = normalizeText(message);
  const businessKnown = /(business|coach|class|course|cook|food|cater|restaurant|bakery|recipe|beauty|hair|salon|clean|service|digital product|store|shop)/.test(text);
  const actions = [
    [/(book|booking|appointment|schedule)/, 'Book'],
    [/(order|ordering)/, 'Order'],
    [/(buy|purchase|checkout|sell)/, 'Buy'],
    [/(quote|estimate)/, 'Request a Quote'],
    [/(call|phone)/, 'Call'],
    [/(text|sms)/, 'Text'],
    [/(email|e-mail)/, 'Email']
  ].filter(([pattern]) => pattern.test(text)).map(([, label]) => label);
  const mediaKnown = /(photo|photos|image|images|video|videos|gallery|media)/.test(text);
  const noAi = /(no ai|no video|do not need ai|don't need ai|without ai)/.test(text);
  const wantsAi = !noAi && /(ai video|video studio|heygen|include ai|want ai)/.test(text);
  const pageKnown = /(one page|1 page|single page|full website|multiple pages|multi-page)/.test(text);
  if (!businessKnown || !actions.length || !mediaKnown || (!noAi && !wantsAi) || !pageKnown) return '';

  let plan = 'Starter Pro';
  let reason = 'It supports photo and video uploads and up to 2 customer action buttons.';
  let extra = '';
  if (wantsAi) {
    plan = 'Business';
    reason = 'It includes AI Video Studio access and supports more sections and customer action buttons.';
    if (/(one page|1 page|single page)/.test(text)) extra = " A lower-cost option is Starter Pro plus Cookie's $5 standalone AI Video Studio.";
  } else if (actions.length > 4) {
    plan = 'Premium';
    reason = 'It supports up to 8 customer action buttons and gives you the most flexibility.';
  } else if (actions.length > 2 || /(full website|multiple pages|multi-page)/.test(text)) {
    plan = 'Business';
    reason = 'It supports more sections and up to 4 customer action buttons.';
  }
  return `Based on your answers, I recommend ${plan}. You need ${actions.join(' and ')}, photo or media uploads, ${noAi ? 'no AI Video Studio' : 'AI Video Studio'}, and ${/(one page|1 page|single page)/.test(text) ? 'one page' : 'a fuller website'}. ${reason}${extra}`;
}

export function fallbackAnswer(message = '', pagePath = '', siteContext = null) {
  ...
  if (intent === 'plan_help') {
    const directRecommendation = planAnswerFromMessage(message);
...
5. Is this one page or a fuller website?`;
  ...
  if (intent === 'writing_help') {
    const business = String(siteContext?.businessName || '').trim() || 'Your Business';
    const description = String(siteContext?.description || '').trim();
    return `Ready-to-paste homepage draft:\n\nHeadline: Welcome to ${business}\n\nDescription: ${description || `${business} makes it simple for customers to learn what you offer and take the next step.`}\n\nCall to action: Explore our services and choose the best way to connect, book, order, or buy.\n\nTell me the main service or product and what makes it special, and I will personalize this wording further.`;
  }

  if (intent === 'ai_video') {
    return "Cookie's AI Video Studio helps with video ideas, hooks, scripts, captions, scenes, prompts, and content planning. The verified $5 one-time standalone purchase includes exactly 1 real AI-generated video. Business and Premium may include monthly real-video access based on plan limits. Generation is subject to provider processing, moderation, and availability.";
  }
