...
export function websiteVideoEntitlement(website = {}, { now = new Date(), limits = configuredVideoLimits() } = {}) {
  ...
  const accessStatus = clean(website.access_status || embedded.access_status || '');
  const subscriptionStatus = clean(website.subscription_status || embedded.subscription_status || '');
  const active = accessStatus === 'active' && subscriptionStatus === 'active' &&
    !['paused', 'archived', 'deleted', 'inactive'].includes(status);
