...
import { isCanonicalOwnerEmail, normalizeOwnerEmail } from './adminAuth.mjs';
...
export function ownerEmail(user = {}) {
  return normalizeOwnerEmail(user?.email);
}
...
export function adminSessionCookieOptions(maxAge = 60 * 60) {
  return {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'lax',
...
export function isAllowedAdminEmail(email = '') {
  return isCanonicalOwnerEmail(email);
}
...
export async function getVerifiedAdmin(req) {
  ...
  const email = ownerEmail(data?.user);
  if (error || !data?.user || !email) {
    return { ok: false, status: 401, error: 'Your owner session expired. Sign in again with your owner password.' };
  }
