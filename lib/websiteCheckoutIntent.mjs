...
export function websiteCheckoutSubjectRef(value = '') {
  const subject = String(value || '').trim();
  return subject ? createHash('sha256').update(`website-checkout-subject:${subject}`).digest('hex').slice(0, 12) : 'missing';
}
...
export function traceWebsiteCheckout(event = '', intent = {}, details = {}) {
  ...
  console.info('[paid-checkout-trace]', {
    ...
    websiteRef: websiteCheckoutSubjectRef(details.websiteId || intent?.website_id || intent?.draft_slug),
    ownerRef: websiteCheckoutSubjectRef(details.ownerId || intent?.owner_id),
    storedPlan: normalizeWebsiteCheckoutPlan(details.storedPlan) || undefined,
    builderPlan: normalizeWebsiteCheckoutPlan(details.builderPlan) || undefined,
    browserPlan: normalizeWebsiteCheckoutPlan(details.browserPlan) || undefined,
    intentPlan: normalizeWebsiteCheckoutPlan(details.intentPlan || intent?.plan) || undefined,
    product: normalizeWebsiteCheckoutPlan(details.product) || undefined,
    entitlementState: String(details.entitlementState || '').replace(/[^a-z0-9_-]/gi, '').slice(0, 40) || undefined,
...
export function newWebsiteCheckoutIntent({ id = randomUUID(), plan = '', draftSlug = '', email = '', ownerId = '', websiteId = '', now = Date.now() } = {}) {
  ...
  return {
    ...
    website_id: String(websiteId || '').trim() || null,
...
export function checkoutIntentBelongsToOwner(row = {}, owner = {}) {
  ...
  return checkoutIntentIdentityBelongsToOwner(row, owner);
}

export function checkoutIntentIdentityBelongsToOwner(row = {}, owner = {}) {
  const ownerId = String(owner.user?.id || owner.id || '').trim();
  const emailHash = checkoutIntentEmailHash(owner.email || '');
  const intentOwnerId = String(row.owner_id || '').trim();
  const intentEmailHash = String(row.email_hash || '').trim();
  if (intentOwnerId && intentOwnerId !== ownerId) return false;
  if (intentEmailHash && intentEmailHash !== emailHash) return false;
