...
export function filterAuthorizedVideoJobs(jobs = [], { access, authorized, requestedEmail = '' } = {}) {
  ...
  if (!email) return [];
  if (standaloneIdentityMatches(access, email)) return scoped;
  return scoped.filter(row => normalizeVideoEmail(row?.customer_email) === email);
}
...
export function authorizeVideoResultAccess({ access, owner = null, requestedEmail = '', requestedSlug = '', requireIdentity = false } = {}) {
  ...
  if (access.kind === 'standalone') {
    ...
    const hasIdentity = Boolean(access.emailHash || normalizeVideoEmail(access.email));
    if (!hasIdentity || !allowedSlug) return { ok: false, status: 403 };
    if (requireIdentity && !email) return { ok: false, status: 403 };
    return { ok: true, slug: allowedSlug };
  }
