...
export const APPROVED_WEBSITE_PRODUCTS = Object.freeze({
  ...
  extra: Object.freeze({
    plan: 'extra_page',
    productId: 'MQy2SypFmCSvQz4e4BJgRg==',
...
export const PROTECTED_NON_SUBSCRIPTION_PRODUCTS = Object.freeze({
  aiVideo: Object.freeze({
    productId: 'GE_fDgvz_GT29Fn6eSj9uw==',
    permalink: 'aivideostudio'
  }),
  dfyExtraPage: Object.freeze({
    productId: 'i3wPER_6xJZXR7hHSjSL1Q==',
    permalink: 'dfy-extra-page-addon',
    shortProductId: 'ctuek'
  })
});
...
export function sanitizeGumroadPayload(payload = {}) {
  ...
  const allowed = new Set([
    ...
    'sale_timestamp', 'created_at', 'recurrence', 'charge_occurrence_count',
    'cancelled', 'cancelled_at', 'cancelled_by_admin', 'cancelled_by_buyer',
    'cancelled_by_seller', 'cancelled_due_to_payment_failures',
    'user_requested_cancellation_at', 'ended_at', 'ended_reason', 'failed_at',
    'restarted_at', 'effective_as_of', 'type', 'partially_refunded'
  ]);
  for (const [key, value] of Object.entries(payload)) {
    const normalizedKey = key.toLowerCase();
    const isWebsiteField = normalizedKey.includes('website') || normalizedKey.includes('subdomain');
    if (!allowed.has(key) && !isWebsiteField) continue;
